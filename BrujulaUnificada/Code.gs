...
function onOpen() {
  ...
  ui.createMenu('🚀 Navi Admin')
    .addItem('1. Inicializar Tablas', 'setupNaviSheets')
    .addSeparator()
    .addSubMenu(ui.createMenu('🗄️ Legacy (BrujulApp)')
      .addItem('L1. Generar Enlaces Personalizados', 'generateCustomLinks')
      .addItem('L2. Enviar Invitacion Inicial', 'sendInitialInvitationEmails')
      .addItem('L3. Enviar Recordatorios (Pendientes)', 'sendAutomaticReminders')
      .addItem('L4. Enviar Prueba Invitación', 'sendSingleTestEmail')
      .addItem('L5. Enviar Prueba Reminder', 'sendSingleReminderTestEmail')
      .addItem('L6. Enviar Prueba Followup', 'sendSingleFollowupTestEmail')
      .addSeparator()
      .addItem('L7. Ver Resumen Observabilidad V1', 'showObservabilitySummary')
      .addItem('L8. Ver Resumen de Envíos', 'showDeliverySummary')
      .addItem('L9. Poblar Campos Base (Students)', 'populateStudentsBaseFields')
      .addSeparator()
      .addItem('L10. Enviar Seguimiento Brujula', 'sendFollowupEmailBrujula')
      .addItem('L11. Inicializar GoalBank', 'setupGoalBankSheet')
    )
    .addToUi();
}
...
/** 
 * ==========================================
 * NAVI SYNC API (Dashboard Integration)
 * ==========================================
 */
...
function doPost(e) {
  try {
    ...
    if (action === 'updateStudent') {
      return updateStudentStatusNavi(ss, params.data);
    }
    
    if (action === 'syncBulk') {
      return syncBulkDataNavi(ss, params.data);
    }

    return ContentService.createTextOutput(JSON.stringify({ 
      status: 'error', 
      message: 'Acción no reconocida por Navi' 
...
function handleNaviGetData() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const studentsData = ss.getSheetByName('Students').getDataRange().getValues();
    const mentorsData = ss.getSheetByName('Mentors').getDataRange().getValues();
    
    return ContentService.createTextOutput(JSON.stringify({
      status: 'success',
      data: {
        students: parseNaviSheetData(studentsData),
        mentors: parseNaviSheetData(mentorsData)
      }
    })).setMimeType(ContentService.MimeType.JSON);
    ...
    return ContentService.createTextOutput(JSON.stringify({
      status: 'error',
      message: 'Error en Navi GET: ' + err.toString()
    })).setMimeType(ContentService.MimeType.JSON);
  }
}

/** Auxiliar: Convierte matriz de Sheet a Array de Objetos para Navi */
function parseNaviSheetData(values) {
  if (values.length <= 1) return [];
  const headers = values[0];
  return values.slice(1).map(row => {
    let obj = {};
    headers.forEach((h, i) => {
      // Normalizar nombres de llaves para JS (ej. "#HEX" -> "hex")
      const key = h.toLowerCase().replace(/[^a-z0-0]/g, '');
...
function syncBulkDataNavi(ss, data) {
  const studentSheet = ss.getSheetByName('Students');
  const mentorSheet = ss.getSheetByName('Mentors');
  
  // Limpiar y escribir Mentores
  if (!mentorSheet) ss.insertSheet('Mentors');
  const mSheet = ss.getSheetByName('Mentors');
  mSheet.clearContents();
  mSheet.appendRow(['Comunidad', '#HEX', 'Nombre', 'Nickname', 'Email']);
  data.mentors.forEach(m => {
    mSheet.appendRow([m.community, m.hex, m.name, m.nickname, m.email]);
  });
  
  // Limpiar y escribir Alumnos
  if (!studentSheet) ss.insertSheet('Students');
  const sSheet = ss.getSheetByName('Students');
  sSheet.clearContents();
  sSheet.appendRow(['Matrícula', 'Nombre', 'Email', 'NicknameMentor', 'Comunidad', 'Status', 'Check-in']);
  data.students.forEach(s => {
    sSheet.appendRow([s.matricula, s.name, s.email, s.mentor, s.community, s.status, s.checkIn]);
  });
...
function updateStudentStatusNavi(ss, data) {
  const sheet = ss.getSheetByName('Students');
  const values = sheet.getDataRange().getValues();
  const matricula = data.matricula;
  
  for (let i = 1; i < values.length; i++) {
    if (values[i][0] == matricula) {
      if (data.status) sheet.getCell(i + 1, 6).setValue(data.status);
      if (data.checkIn) sheet.getCell(i + 1, 7).setValue(data.checkIn);
      return ContentService.createTextOutput(JSON.stringify({ status: 'success' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
  }
  return ContentService.createTextOutput(JSON.stringify({ status: 'error', message: 'Alumno no encontrado' }))
    .setMimeType(ContentService.MimeType.JSON);
}

/** Inicializa la estructura de hojas y encabezados */
function setupNaviSheets() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  if (!ss.getSheetByName('Mentors')) {
    const s = ss.insertSheet('Mentors');
    s.appendRow(['Comunidad', '#HEX', 'Nombre', 'Nickname', 'Email']);
  }
  if (!ss.getSheetByName('Students')) {
    const s = ss.insertSheet('Students');
    s.appendRow(['Matrícula', 'Nombre', 'Email', 'NicknameMentor', 'Comunidad', 'Status', 'Check-in']);
  }
  SpreadsheetApp.getUi().alert('✅ Estructura de Navi lista en el Sheet.');
}
